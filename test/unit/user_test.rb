...
class UserTest < ActiveSupport::TestCase
  ...
  def test_require_username
    assert_equal [I18n.t('errors.messages.empty')], new_user(:username => '').errors[:username]
  end

  def test_require_password
    assert_equal [I18n.t('errors.messages.empty')], new_user(:password => '').errors[:password]
  end
  ...
  def test_validate_uniqueness_of_email
    ...
    assert_equal [I18n.t('errors.messages.taken')], new_user(:email => 'bar@example.com').errors[:email]
  end

  def test_validate_uniqueness_of_username
    new_user(:username => 'uniquename').save!
    assert_equal [I18n.t('errors.messages.taken')], new_user(:username => 'uniquename').errors[:username]
  end
  ...
  def test_validate_password_length
    assert_equal [format(I18n.t('errors.messages.too_short'),{:count => 4})], new_user(:password => 'bad').errors[:password]
  end

  def test_require_matching_password_confirmation
    assert_equal [I18n.t('errors.messages.confirmation')], new_user(:password_confirmation => 'nonmatching').errors[:password]
  end
